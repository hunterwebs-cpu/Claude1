...
## TITLE RESOLVED, ALL FIVE OTHER FILINGS CONVERTED, CROSS-REFERENCES FIXED,
FORMATTING SKILL BUILT — and two real formatting bugs shipped and had to be
corrected in front of the client (2026-08-17)

Client's instructions, verbatim: "I want our final formatting agent to have
a full skill to build my briefs this way after the substantive write is
done." / "all the oth3r filings need to be uodted" / "one thing, since I
didnt refer to arguments by point I or poi ii, the areas that reference the
arguments need to be updated." / "as a PDF please" / on the title framing
question: chose "MEMORANDUM OF LAW IN SUPPORT OF A LEVEL TWO RISK
CLASSIFICATION" over "...MOTION FOR DOWNWARD DEPARTURE" and over an
opposition-framed alternative.

**Title finalized.** Caption's title block now reads "MEMORANDUM OF LAW /
IN SUPPORT OF A LEVEL TWO / RISK CLASSIFICATION" — client's own choice,
correctly avoiding the "downward departure" framing flagged as
inconsistent with the memo's actual argument (insufficient proof on the
public-safety element, not a discretionary departure from an
otherwise-sound Level Three).

**Cross-references fixed.** Since the client's own rebuild of the ARGUMENT
section dropped "POINT I"/"POINT II" entirely (each Point's title is now
itself a lettered heading directly under ARGUMENT — "A." and "B." — with
sub-subsections numbered 1., 2., 3...), every internal and cross-document
reference to the old labels was updated to match:
- Old Point I → Argument A
- Old Point II → Argument B
- Old Point II.A/.B/.D/.E → Argument B.1/.2/.4/.5
Fixed in the memo itself (7 instances) and in the § 722-c motion (5
instances), which cites the memo's arguments by section. Also fixed three
stale references to the memo's *old* title ("...in Support of
Respondent's Position") in the Certification, the § 722-c motion, and the
Demand for Disclosure, to the new title.

**All five other filings converted to the memo's format.** Built a
general-purpose converter (`tools/md_to_ny_filing_docx.js`, also the basis
for the new skill's reference script) that produces the same caption table,
Roman-numeral/letter heading scheme, single-spaced headings, and
double-spaced indented body text as the memo, parameterized per document's
own title and (unchanged across filings) case caption. Converted: the
Certification, the Adjournment Motion, the Counsel Application, the § 722-c
Motion, and the Demand for Disclosure. All five delivered as both `.docx`
and `.pdf` (PDFs generated by converting the `.docx` via LibreOffice, so the
two are pixel-identical — no separate PDF pipeline to drift out of sync).

**Two real formatting bugs shipped before being caught — both flagged
plainly here rather than glossed over:**

1. **Doubled solid lines in the caption**, on every one of the five newly
   converted filings (and, on inspection, in the memo's caption too, from
   the same underlying cause). A paragraph border was applied to a
   paragraph that ALSO contained a literal dashed-text rule line
   (`-----------------------------------------------------X`), so both the
   real border and the dashes rendered — two stacked lines where the
   client's own design has one. Client's correction, verbatim: "THOSE 2
   FUCKING LONES ACROSS THE PAGE ARE STILL FUCKING THERE and they are in
   EVERY FUCJING FILE!" Fixed by removing every stray `<w:pBdr>` from the
   caption region of all six documents (memo included), leaving only the
   literal dashed lines as the visual rule, exactly matching the client's
   original hand-built design. The NOTES/footnote section divider (a
   single, non-duplicated border) was left alone — it was never part of
   the problem.
2. **Orphaned title-wrap word**, in the memo's new title specifically: a
   34-character line estimate for the bold title text underestimated how
   much room bold 12pt Times New Roman actually needs in the caption's
   ~3.25" title column, so "MEMORANDUM OF LAW IN SUPPORT OF A" re-wrapped
   inside the table cell, stranding "OF A" alone on its own line with a
   visible gap above it. Fixed by re-splitting the title into shorter,
   safely-conservative lines (≤28 characters) and, for the general
   converter, lowering the default wrap width for all future titles from
   34 to 28 characters.

**Both bugs are now written up as named pitfalls, with the specific fix, in
the new formatting skill** — not just fixed in this matter's documents, so
they don't recur on a future brief.

**New skill created**: `.claude/skills/brief-formatting/SKILL.md` (project
level, general-purpose — not Bernecky-specific), with a reference converter
script at `.claude/skills/brief-formatting/scripts/md_to_filing_docx.js`.
Covers: caption-as-table construction, the Roman/letter/number heading
scheme (including the "don't invent a POINT I/II-style parallel numbering
system" rule and the "never leave a heading unlabeled" rule), spacing rules,
docx→PDF conversion via LibreOffice as the single source of truth, and a
four-part verification protocol (schema validation, word-count integrity,
**actually rendering and reading the result**, and a stale-cross-reference
grep) — the last two items exist specifically because of the two bugs above.

**Verification performed on the corrected final versions:** OOXML schema
validation passed on all 6 documents; word-count integrity ratios ~0.997-1.00
across all 6 (no dropped content); every document's first page, last page,
and (where applicable) lettered ARGUMENT section rendered and visually
reviewed, confirming no stray border lines anywhere and no wrapped/orphaned
title text.
