...
**Persistent identity:**
- Session ID: `session_01Mx6Fab4VarTsqMgomWNUMA`
- Tags: `litigator`, `vince`, `standing-agent` — find him anytime via `list_sessions` (filter by tag, or search titles for "Vince")
- Every task must tell him which repo/branch/case is live; he uses `add_repo` to attach it himself. Do not assume he's already looking at any particular matter — always name it.
- **No `send_message` tool is available in this environment to message an existing CCR session with a follow-up task.** Each new assignment currently requires archiving the prior session and recreating "Vince" fresh via `create_session`, with the standing brief plus the new task combined into the initial prompt. (Earlier sessions, now archived: `session_01P2bK8RuXGx6M7u9vUpsLTo` — Bernecky-bound at creation; `session_01TS5m8DygLGkokrMDY2BzzT` — first general-purpose version, replaced after a process failure, see below.)

**Process failure and correction (2026-08-10):** The first Part I.D task assignment gave Vince nearly-finished argument sentences to work from instead of raw facts and law — effectively dictating the argument rather than letting him build it. He faithfully reproduced a real reasoning error in that dictated text (treating "Congress" as the drafter of an individual supervised-release condition, when sentencing courts, not Congress, draft the specific conditions in a judgment; Congress only authorizes the statutory menu a court may draw from). The client caught this and correctly diagnosed it as the paralegal turning Vince into a "yes-man" rather than independent counsel. Corrected by relaunching him with: raw source documents only (verbatim Judgment text, verbatim CIMP Agreement, the full text of *Kunz* pasted in rather than paraphrased, client-supplied facts as facts, existing research memos flagged as unverified leads to check himself); an explicit instruction that he — not the paralegal — decides which sub-arguments to make and how to sequence them; and an explicit instruction to disregard the tainted first draft as a substantive source. **Lesson for future task assignments to Vince or any similar drafting agent: give facts, verified law, and an objective — never pre-written argument text or a dictated structure.**
